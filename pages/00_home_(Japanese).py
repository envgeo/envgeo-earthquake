...
- 海岸線オーバーレイ: EnvGeo の地図ユーティリティから継承したローカル海岸線座標ファイルを、表示上の参照レイヤーとして使用します。
    ...
    with tab_manual:
        ...
        st.markdown(
            """
画面キャプチャ付きマニュアルは、このリポジトリ内にあります。

- 日本語: `docs/manual_Japanese/README.md`
...
7. `断面図` で A-B 断面を定義し、深さ分布と地図上の断面線を確認します。
8. `時系列頻度` で時間的な集中や変化を確認します。
...
- `0.3.1`（未リリース、2026-09-19更新）
  ...
  - **修正:** 読み込み中にSimple / Advancedを切り替えた際の状態衝突を防ぐため、API検索ウィジェットをページ別に分離し、不正な一時範囲を地域プリセット値へ自動復旧。
...
- `0.2.5`（2026-09-16）
  ...
  - **追加:** 作業ログ、テストメモ、リリース前チェックリスト、README 更新方針を含む維持管理文書を追加。
  - **改善:** 実行用・テスト用 requirements を分け、外部データ利用条件の注意を明確化。
